...
class AnimalSorterTest {
    ...
    void shouldNotModifyOriginalList() {
        ...
        List<Animal> sorted = sorter.sortByAge(original);
    ...
    void shouldSortNamesCaseInsensitively() {
        ...
        Animal bomb = new Animal("bOmB", "Cat", 1, true, LocalDate.now());
        Animal alpha = new Animal("Alpha", "Cat", 1, true, LocalDate.now());
        List<Animal> result = sorter.sortByName(List.of(zebra, bomb, alpha));
        assertThat(result).extracting(Animal::getName)
                .containsExactly("Alpha", "bOmB", "Zebra");
    }
